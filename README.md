...
|---|---|---|
| UI 이미지 3장 이상 | ☑ (5장) | 2-1 |
| 화면 역할이 구분되는 구성 | ☑ (진입 / 입력 / 관리 / 결과) | 2-1 |
| 모바일 9:16 또는 데스크탑 16:9 준수 | ☑ (모바일 세로 화면 비율) | 2-1 |
| 파일 형식 PNG 또는 JPG | ☑ (PNG) | 2-1 |
| 이미지 내 텍스트 깨짐·부자연스러운 요소 수정 | ☑ (텍스트 깨짐 없음, 잘못 생성된 요소 3건 수정) | 4-3 |
...
|---|---|---|---|
| 초안 | (없음. 처음 정리한 8개 화면에 진입 화면이 없었다) | — | — |
...
| 단계 | 프롬프트 | 변경 이유 | 결과 및 개선점 |
|---|---|---|---|
| 초안 | 취소표 모니터링 앱 화면 4개를 한 장에 보여주는 보드로 만들어줘. 수단 선택 → 일정 입력 → 세부 조건 → 조건 확인. 파란색 #2563EB, 390×844 | — | 한글은 제대로 나왔지만 네 화면이 한 장에 들어가다 보니 화면 하나당 크기가 너무 작아졌다. 요청하지도 않은 설정(⚙) 아이콘이 위쪽에 생겼고, 입력이 4단계로 쪼개져 화면 수가 늘어났다 |
...
| 단계 | 프롬프트 | 변경 이유 | 결과 및 개선점 |
|---|---|---|---|
| 초안 | 요청 목록 화면. 모니터링 중인 요청 카드를 나열하고 상태 칩과 수정·일시정지·삭제 버튼 배치 | — | 내가 고른 교통수단만 보여서, 다른 수단으로 바꿔 탈 수 있는지 판단할 정보가 화면에 없었다 |
...
| 단계 | 프롬프트 | 변경 이유 | 결과 및 개선점 |
|---|---|---|---|
| 초안 | 알림 목록 화면. 취소표 발견 알림 카드를 시간순으로 나열 | — | 알림 내용이 걸어둔 조건과 상관없이 만들어져서, 어떤 요청의 결과인지 알 수 없었다 |
